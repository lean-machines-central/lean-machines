...
instance [Machine ACTX AM] [Machine CTX M] [instR: Refinement AM M]
  (abs : OrdinaryEvent AM α' β') (ev : OrdinaryEvent M α β) (rf : OrdinaryREvent AM M abs ev):
  SafeREventPO
    (AM := AM) (M := M)
    (α := α) (β := β)
  ...
  where
    lift_in := rf.lift_in
    lift_out := rf.lift_out
    strengthening := rf.strengthening
    simulation := rf.simulation
...
def newREvent [Machine ACTX AM] [Machine CTX M] [Refinement AM M]
  (abs : OrdinaryEvent AM α' β') (ev : OrdinaryEvent M α β)
  (rf: OrdinaryREvent AM M abs ev)
  : OrdinaryREvent AM M abs ev (α := α) (β := β) := rf
...
instance [Machine ACTX AM] [Machine CTX M] [instR: Refinement AM M]
  (abs : InitEvent AM α' β') (ev : InitEvent M α β) (rf : SafeInitREvent AM M abs ev):
  SafeInitREventPO
    (AM := AM) (M := M)
    (α := α) (β := β)
    (ev) (abs.to_InitEvent (M := AM))
    (instSafeAbs := safeInitEventPO_InitEvent abs)
    (instSafeEv := safeInitEventPO_InitEvent ev)
  where
    lift_in := rf.lift_in
    lift_out := rf.lift_out
    strengthening := rf.strengthening

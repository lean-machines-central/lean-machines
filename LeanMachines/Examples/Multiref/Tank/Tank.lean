...
instance : SafeInitREventPO (Tank1.init (ctx:= ctx)) (Xor0.Init (ctx:= ctx')).to_InitEvent where
    lift_in := id
    lift_out := id
...
instance : SafeREventPO
    (instSafeEv := instFillUp.toSafeEventPO)
    (instSafeAbs := instSafeEventPO_OrdinaryEvent Counter0.setCount)
    (Tank1.fillUp (ctx := ctx)) (Counter0.setCount (ctx:= ctx)).toEvent
    (valid_kind :=
        by
            simp
    )
...
instance : SafeREventPO
        (Tank1.fillUp (ctx:= ctx)) (Xor0.SetX_false (ctx:= ctx')).toEvent
        (instSafeEv := instFillUp.toSafeEventPO)
        (instSafeAbs := instSafeEventPO_OrdinaryEvent (Xor0.SetX_false))
        (valid_kind := by simp)
    where
    lift_in := fun _ => ()
    lift_out := id
...
instance : SafeREventPO
    ...
    strengthening := by
        ...
        intros
        simp[newConvergentEvent',newConvergentEvent]
        omega
    simulation m x :=
        by
            simp[Machine.invariant, Refinement.refine, Tank1.drain, Counter0.Decr]
...
instance : SafeREventPO
    (Tank1.drainAll (ctx:=ctx)) (Counter0.setCount (ctx:=ctx)).toEvent
    (instSafeEv := instDrainAll.toSafeEventPO)
    (instSafeAbs := instSafeEventPO_OrdinaryEvent (Counter0.setCount))
    (valid_kind := by simp)
    where
    lift_in :=  fun _ => 0
...
instance : SafeREventPO (Tank1.drainAll (ctx := ctx)) (Xor0.SetY_false (ctx:=ctx')).toEvent
    (instSafeEv := instDrainAll.toSafeEventPO)
    (instSafeAbs := instSafeEventPO_OrdinaryEvent (Xor0.SetY_false))
    (valid_kind := by simp)
    where
    lift_in := fun _ => ()
    lift_out := id
...
instance : SafeREventPO (Tank1.Open_Door_in (ctx := ctx)) (skip_Event (Counter0 ctx) Unit)
...
instance : SafeREventPO (Tank1.Open_Door_in (ctx:= ctx)) (Xor0.SetX_true (ctx:= ctx')).toEvent
    (instSafeEv := instOpenIn)
    (instSafeAbs := instSafeEventPO_OrdinaryEvent Xor0.SetX_true)
    (valid_kind := by simp)
    where
    lift_in := id
    lift_out := id
...
instance : SafeREventPO (Tank1.Close_Door_in (ctx := ctx)) (skip_Event (Counter0 ctx) Unit)
...
instance : SafeREventPO (Tank1.Close_Door_in (ctx:= ctx)) (Xor0.SetX_false (ctx:= ctx')).toEvent
    (instSafeEv := instCloseIn)
    (instSafeAbs := instSafeEventPO_OrdinaryEvent Xor0.SetX_false)
    (valid_kind := by simp)
    where
...
instance : SafeREventPO (Tank1.Open_Door_out (ctx := ctx)) (skip_Event (Counter0 ctx) Unit)
...
instance : SafeREventPO (Tank1.Open_Door_out (ctx:= ctx)) (Xor0.SetY_true (ctx:= ctx')).toEvent
    (instSafeEv := instOpenOut)
    (instSafeAbs := instSafeEventPO_OrdinaryEvent Xor0.SetY_true)
    (valid_kind := by simp)
    where
    lift_in := id
    lift_out := id
...
instance : SafeREventPO (Tank1.Close_Door_out (ctx := ctx)) (skip_Event (Counter0 ctx) Unit)
...
instance : SafeREventPO (Tank1.Close_Door_out (ctx:= ctx)) (Xor0.SetY_false (ctx:= ctx')).toEvent
    (instSafeEv := instCloseOut)
    (instSafeAbs := instSafeEventPO_OrdinaryEvent Xor0.SetY_false)
    (valid_kind := by simp)
